...
VMware NSX network virtualization and security management.
...
get_logical_switches(alias) -> list

# Get logical routers
get_logical_routers(alias) -> list

# Get transport zones
get_transport_zones(alias) -> list

# Get edge clusters
get_edge_clusters(alias) -> list

# Get firewall sections
get_firewall_sections(alias) -> list

# Get firewall rules
get_firewall_rules(alias, section_id=None) -> list

# Get security groups
get_security_groups(alias) -> list

# Get IP pools
get_ip_pools(alias) -> list

# Get NSX controllers
get_controllers(alias) -> list

# Get NSX version information
get_version(alias) -> dict

# Get segments
get_segments(alias) -> list

# Get BGP neighbors
get_bgp_neighbors(alias, tier_id=None) -> list

# Get tier gateways (Tier-0 and Tier-1)
get_tiers(alias, tier_type=None) -> list

# Get alarms
get_alarms(alias) -> list

# Get NTP source configuration
get_ntp_source(alias) -> dict

# Get syslog exporters
get_syslog_source(alias) -> list

# Get LDAP identity sources
get_ldap_source(alias) -> list
----
...
----
# Python
bot = Sysbot("vmware.nsx")
bot.open_session("nsx", "http", "basicauth", "nsx-manager.example.com", 
                443, "admin", "password")

# Get logical switches
switches = bot.modules.vmware.nsx.get_logical_switches("nsx")

# Get firewall rules from a specific section
rules = bot.modules.vmware.nsx.get_firewall_rules("nsx", section_id="1234")
...
----
# Robot Framework
${switches}=    Call Components    modules.vmware.nsx.get_logical_switches    nsx
${routers}=     Call Components    modules.vmware.nsx.get_logical_routers     nsx
${version}=     Call Components    modules.vmware.nsx.get_version             nsx
----
